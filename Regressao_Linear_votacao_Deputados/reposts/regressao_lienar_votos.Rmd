...
Nossos resíduos se desviam um pouco (normalmente não distribuídos). Como temos um desvio relativamente leve, temos um sinal negativo, porém moderado, para o nosso modelo.
...
Embora não seja perfeito, podemos ver que os resíduos desse modelo estão mais próximos de normalmente distribuídos.


## Todas as variáveis são úteis para os modelos de regressão? Há variáveis redudantes?

Para esta pergunta, foram utilizada apenas as variáveis numéricas, já que existem muitas categorias nas variáveis catégoricas.
...
Observando o gráfico de correlações acima de 2006 e 2010 podemos obsevar que as variáveis total_despesa e total_receita são as que mais estão correlacionada com a variável votos em ambos os anos, e que a correlação entre elas mesmas é proximo de 1, o que pode indicar uma redundância ao utiliza-las na regressão, e outras variáveis como recursos_de_pessoas_físicas e recursos_de_pessoas_juridicas possuem uma baixa correlação com votos, o que mostra que elas podem não conseguir explicar os votos e podem ser removidas da regressão, ou seja, nem todas as variáveis são importantes.
...
**Conclusão:** Observamos que nos gráficos de resíduos e Q-Q que se assemelham aos modelos individuais, já para os valores de R2 e RSE observamos que tem uma diferença em relação aos modelos individuais, sendo assim podemos observar que o modelo considerando às duas eleições não gera bons resultados. 