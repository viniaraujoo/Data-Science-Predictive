...
### KNN
```{r}
cross_validation <- trainControl(method = "repeatedcv", number = 5,repeats=5)
...
model_logistic<- train(situacao ~ ., 
                     ...
                     method = "regLogistic",
                     preProc = c("center", "scale"),
                     trControl = trainControl(method = "optimism_boot", classProbs = TRUE),
                     tuneGrid = rlGrid)
plot(model_logistic)
model_logistic
```
...
```{r}
cross_validation <- trainControl(method = "boot", number = 5,classProbs = TRUE)
...
```{r}
cross_validation <- trainControl(method = "boot", number = 3,repeats = 5,sampling = "smote", search = "random", verboseIter = FALSE)
